...
@blueprint.periodic(cron="10 * * * *")  # pyrefly: ignore[bad-argument-type]
...
@blueprint.periodic(cron="15 * * * *")  # pyrefly: ignore[bad-argument-type]
...
async def remove_old_jobs(context: JobContext, timestamp: int):
    return await builtin_tasks.remove_old_jobs(  # pyrefly: ignore[invalid-param-spec]
        context,
        max_hours=1,
        remove_failed=True,
