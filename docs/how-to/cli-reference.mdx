---
title: "CLI Reference"
description: "Complete reference for the Agent Stack command-line interface"
---

Agent Stack command-line interface (CLI) provides powerful tools to discover, run, and manage agents directly from your terminal. This document serves as a comprehensive reference for all available commands and options.

## Basic Usage

The basic syntax for Agent Stack commands follows this pattern:

```bash
agentstack [command] [subcommand] [arguments] [options]
```

To see a list of all available commands:

```bash
agentstack --help
```

## Core Commands

### List

List all available agents:

```bash
agentstack list
```

This displays a table with agent names, statuses, descriptions, UI types, locations, missing environment variables, and error messages.

### Run

Run an agent with specified input:

```bash
agentstack run <agent-name> [input]
```

**Examples:**
```bash
# Run chat agent interactively
agentstack run chat

# Run agent with direct input
agentstack run chat "Hello!"

# Run agent with input from file
agentstack run chat < input.txt

# Pipe between agents
agentstack run gpt_researcher "Latest AI developments" | agentstack run chat "Summarize in 3 points"
```

### Info

Get detailed information about a specific agent:

```bash
agentstack info <agent-name>
```

Shows details including description, input/output schemas, examples, configuration options, and provider information.

### Add

Add an agent to your environment:

```bash
agentstack add <location>
# Aliases: install
```

Use `agentstack add` when:
- You want to use an agent (one-step process)
- You don't care about build details
- You want the simplest workflow

Location formats:

```bash
# From Docker image
agentstack add ghcr.io/i-am-bee/agentstack/agents/chat:latest

# From GitHub repository
agentstack add "https://github.com/i-am-bee/custom-agent"

# From GitHub with version
agentstack add "https://github.com/i-am-bee/custom-agent#v1.0.0"

# From GitHub with specific path
agentstack add "https://github.com/i-am-bee/repo#:agents/my-agent"

# From local path (builds Docker image)
agentstack add ./my-agent-directory
```

### Remove

Remove an agent from your environment:

```bash
agentstack remove <agent-name>
# Aliases: uninstall, rm, delete
```

### Logs

View and follow the logs for an agent:

```bash
agentstack logs <agent-name>
```

This displays the agent's log output and continues streaming new logs as they are generated.

### Build

Build an agent from a local directory or repository:

```bash
agentstack build [context]
```

Arguments:
- `context`: Docker context for the agent (default: current directory)

Options:
- `--tag <name>`: Docker tag for the agent
- `--multi-platform`: Build for multiple platforms
- `--import / --no-import`: Import the image into Agent Stack (default: true)
- `--dockerfile=./Containerfile`: Specify your container file

Use `agentstack build` when:
- You want to test builds before using them (`--no-import`)
- You need custom build options (`--multi-platform`, custom `--tag`)
- You're developing and want to control the build process
- You want to build images for sharing (not immediate use)

Examples:

```bash
# Build from current directory
agentstack build .

# Build with custom tag
agentstack build . --tag my-agent:latest

# Build without importing
agentstack build . --no-import

# Build from GitHub URL
agentstack build "https://github.com/user/repo"
```

## Environment Management

### Environment Setup

Interactive setup for LLM provider environment variables:

```bash
agentstack model setup
```

This guides you through setting up the required environment variables for your chosen LLM provider.

### Environment Variables

Manage environment variables for agents:

```bash
# List all environment variables
agentstack env list

# Add or update an environment variable
agentstack env add KEY=VALUE

# Remove an environment variable
agentstack env remove KEY
```

**Examples:**
```bash
# Set up an OpenAI-compatible API
agentstack env add LLM_MODEL=gpt-4o
agentstack env add LLM_API_BASE=https://api.openai.com/v1
agentstack env add LLM_API_KEY=sk_...
```

## System Commands

### Start

Start the Agent Stack server:

```bash
agentstack platform start
```

### Stop

Stop the Agent Stack server:

```bash
agentstack platform stop
```

### Delete

Delete the Agent Stack server:

```bash
agentstack platform delete
```

### UI

Launch the web interface:

```bash
agentstack ui
```

This runs the server in the foreground.

### Version

Display version information:

```bash
agentstack version
```

## Global Options

Most commands support these general options:
- `--help`: Show help information for a command
- `--debug`: Enable debug output for troubleshooting
