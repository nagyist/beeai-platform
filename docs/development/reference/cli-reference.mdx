...
List user feedback for your agents:

```bash
agentstack agent feedback list [agent-name] [OPTIONS]
```

**Options:**
- `--limit <number>`: Number of results per page (default: 50)
- `--after <cursor>`: Cursor for pagination to fetch next page
...
# Filter by agent
agentstack agent feedback list chat

# Limit results
agentstack agent feedback list --limit 10

# Paginate through results
agentstack agent feedback list --after <cursor-token>

# Combine filtering and pagination
agentstack agent feedback list chat --limit 10 --after <cursor-token>
```
