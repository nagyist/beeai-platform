...
Agent Stack supports OIDC authentication either using the bundled Keycloak instance or an external provider.
...
If you want to use an external OIDC provider (e.g., Auth0, Okta, or your own Keycloak), you can disable the bundled Keycloak and configure the external provider details.
...
You can add LLM/embedding models through the CLI (as admin user) or statically in the deployment

```yaml
# make sure these models are available in modelProviders
defaultLLMModel: "openai:gpt-5-mini"
defaultEmbeddingModel: "openai:text-embedding-3-small"

modelProviders:
  - name: "OpenAI"
    type: "openai"
    description: "OpenAI project 'my-project'"
...
- redirect URI should be the public URL + `/api/auth/callback` (e.g. `https://example.com.com/api/auth/callback`)
